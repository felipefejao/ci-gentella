...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Detalhes de Ticket</h1>
        </div>
    </div><!--/.row-->

    <div class="row" ng-init="getTicketsbyID()">
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <div class="form-group col-md-2">
                        <label>Nº Ticket</label>
                        <input type="text" class="form-control text-right" style="font-size: 2vh" id="txtID" value="{{TicketByID.id_ticket}}" ng-disabled="true">
                    </div>
                    <div class="form-group col-md-12">
                        <label>Assunto</label>
                        <input type="text" class="form-control" id="txtDesc" value="{{TicketByID.desc_ticket}}" ng-disabled="true">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Data de Abertura</label>
                        <input type="datetime" class="form-control"  value="{{formatDate(TicketByID.datcad_ticket)}}" ng-disabled="true">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Data de Última Atualização</label>
                        <input type="datetime" class="form-control"  value="{{formatDate(TicketByID.datatu_ticket)}}" ng-disabled="true">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Status Atual</label>
                        <input type="text" class="form-control"  value="{{getStatus(TicketByID.status_ticket)}}" ng-disabled="true">
                    </div>
                    <div class="form-group col-md-12">
                        <label>Histórico</label>
                        <textarea class="form-control" id="txtHistorico"  rows="7" ng-disabled="true">{{TicketByID.obs_ticket}}</textarea>
                    </div>
                    <div class="form-group col-md-12">
                        <label>Procedimento</label>
                        <textarea class="form-control" id="txtObs" ng-model="txtObs" rows="10" ng-disabled="isEnabled(TicketByID.status_ticket)"></textarea>
                    </div>
                    <div class="form-group col-md-12">
                        <button class="btn btn-primary" ng-click="changeTicket()" ng-disabled="isEnabled(TicketByID.status_ticket)">Salvar</button>
                        <button ng-hide="isFinalizarVisible()" class="btn btn-danger" ng-click="salvarTicket()" >Finalizar</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    setTimeout(function () {
        $('#txtHistorico').scrollTop($('#txtHistorico')[0].scrollHeight);
    },200);
